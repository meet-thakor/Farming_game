...
class Crop:
    def __init__(self, name, growth_time):
        ...
        self.age = 0
        self.state = "empty"  # "empty", "planted", "watered", "harvested"
...
class Farm:
    ...
    def plant_crop(self, x, y, crop_name):
        if self.grid[y][x] is None:
            crop = Crop(crop_name, CROP_GROWTH_TIMES[crop_name])
            if crop.plant():
                self.grid[y][x] = crop
                return True
        return False
    ...
    def draw(self, screen):
        for y in range(GRID_HEIGHT):
            for x in range(GRID_WIDTH):
                ...
                if crop is not None:
                    label = font.render(crop.name, True, BLACK)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:  # Left click to plant or water
        ...
        display_message("Left click to plant or water, Right click to harvest", BLACK, (10, 10))
